...
namespace tjp_tester
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            SetBarcode(tjp.QRecLevel.QR_ECLEVEL_L, 2);
        ...
        private void loadInitBitmapToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (LoadInitBitmapForm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                    TjpErrors.Show(tjp.LoadInitBitmap(ofd.FileName, LoadInitBitmapForm.Par_BitmapID));
            }
        }
        ...
        private void getInitStringToolStripMenuItem_Click(object sender, EventArgs e)
        {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    TjpErrors.Show(tjp.EndInit(ofd.FileName, room, size));
        ...
        private void getToolStripMenuItem_Click(object sender, EventArgs e)
        {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    uint w10 = 0;
                    uint h10 = 0;
                    if ( tjp.GetDimensions( ofd.FileName, ref w10, ref h10) != 0) {
        ...
        private void setbarcodeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (SetBarCodeForm.ShowDialog() == DialogResult.OK)
                TjpErrors.Show(
                    SetBarcode(
                        (tjp.QRecLevel)SetBarCodeForm.Par_QRCorrection, 
        ...
        private int SetBarcode( tjp.QRecLevel rl, byte Version) {
            byte[] Setting = new byte[4];
            Setting[0] = (byte)tjp.Encoder.QR;
            Setting[1] = (byte)rl;
            Setting[2] = QRVersion = Version;
            Setting[3] = (byte)tjp.QRencodeMode.QR_MODE_8;

            toolStripStatusLabel1.Text = rl.ToString();
            toolStripStatusLabel2.Text = "QR Version:" + Version.ToString(); 
            return tjp.SetBarcode(Setting);
        }
        ...
        //{
        //    SaveFileDialog sfd = new SaveFileDialog();
        //    sfd.FileName = "barcode.bmp";
        //    sfd.Filter = "Bitmap Image|*.bmp|All Files|*.*";
        //    if (sfd.ShowDialog() == DialogResult.OK)
        //    {
        //        string file = Path.GetFileNameWithoutExtension(sfd.FileName);
        //        for (int f = 0; f != 1000; f++)
        //            tjp.GenerateBarcode( file + "-" + f.ToString() + ".bmp", "Rototype-" + f.ToString(), 1);
        //    }

        //        //TjpErrors.Show(tjp.GenerateBarcode(sfd.FileName, "Rototype", 1));
        //}
